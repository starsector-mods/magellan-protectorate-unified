package data.scripts.magellan.industries;
...
/**
 * Reused for Tichel Distribution Hub
 */
public class MagellanFleetHQ extends BaseIndustry implements RouteFleetSpawner, FleetEventListener {

	private static float OFFICER_PROB_MOD_HIGH_COMMAND = 0.3f;	
	private static float OFFICER_PROB_MOD_TICHEL = 0.1f;	
	private static float DEFENSE_BONUS_COMMAND = 0.3f;
	private static float DEFENSE_BONUS_TICHEL = 0.1f;
	private static float TICHEL_IMPROVE_ACCESSIBILITY = 0.2f;
	private static int IMPROVE_NUM_PATROLS_BONUS = 1;
	
	private static String BLACKCOLLARS_ID = "magellan_blackcollar";
	private static String YELLOWTAILS_ID = "magellan_yellowtail";
	private static String MAGELLAN_ID = "magellan_protectorate";
	private static String TICHEL_HQ = "magellan_tichelhq";
	private static String FLEET_HQ = "magellan_fleethq";
	
	private boolean isTichel() {
		return this.id.equals(TICHEL_HQ);
	}
	...
	public void apply() {
		...
		if (isTichel()) {
			...
			market.getAccessibilityMod().modifyFlat(getModId(0), TICHEL_IMPROVE_ACCESSIBILITY, desc);
		}
		...
		if (isTichel()) bonus = DEFENSE_BONUS_TICHEL;
		...
		if (isTichel()) officerProb = OFFICER_PROB_MOD_TICHEL;
	...
	public void unapply() {
		...
		if (isTichel()) market.getAccessibilityMod().unmodifyFlat(getModId(0));
		else unmodifyStabilityWithBaseMod();
	...
	protected void addPostDemandSection(TooltipMakerAPI tooltip, boolean hasDemand, IndustryTooltipMode mode) {
		if (mode != IndustryTooltipMode.NORMAL || isFunctional()) {
			float opad = 10f;
			if (isTichel()) tooltip.addPara("Accessibility Bonus: %s", opad, Misc.getHighlightColor(), "20%");
			else addStabilityPostDemandSection(tooltip, hasDemand, mode);
			float bonus = DEFENSE_BONUS_COMMAND;
			if (isTichel()) bonus = DEFENSE_BONUS_TICHEL;
	...
	protected Pair<String, Integer> getStabilityAffectingDeficit() {
		if (isTichel()) return new Pair<String, Integer>(Commodities.SUPPLIES, 0);
		else return getMaxDeficit(Commodities.SUPPLIES, Commodities.FUEL, Commodities.SHIPS);
	}
	...
	public CampaignFleetAPI spawnFleet(RouteData route) {
		...
		if (market.getFactionId().equals(MAGELLAN_ID) && !isTichel()) {
			float odds = 0f;
			if (type == PatrolType.COMBAT) odds = 0.5f;
			else if (type == PatrolType.HEAVY) odds = 0.2f;
			if ((float) Math.random() < odds) factionId = BLACKCOLLARS_ID;
			else factionId = market.getFactionId();
		} 
		else if (market.getFactionId().equals(MAGELLAN_ID) && isTichel()) {
			factionId = YELLOWTAILS_ID;
		}
	...
	public static CampaignFleetAPI createPatrol(PatrolType type, String factionId, RouteData route, MarketAPI market, Vector2f locInHyper, Random random) {
		...
		if (fleet.getFaction().getId().equals(BLACKCOLLARS_ID) || fleet.getFaction().getId().equals(YELLOWTAILS_ID)) {
