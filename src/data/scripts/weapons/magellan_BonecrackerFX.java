...
public class magellan_BonecrackerFX
...
EveryFrameWeaponEffectPlugin {
    ...
    public void onFire(DamagingProjectileAPI proj, WeaponAPI weapon, CombatEngineAPI engine) {
        ...
        for (int j = 0; j < shotCount1; ++j) {
            ...
            Vector2f randomVel = randomPointOnCircumference = MathUtils.getRandomPointOnCircumference((Vector2f)null, (float)MathUtils.getRandomNumberInRange((float)18.0f, (float)24.0f));
            randomPointOnCircumference.x += proj_vel.x + ship_vel.x;
            Vector2f vector2f = randomVel;
            vector2f.y += proj_vel.y + ship_vel.y;
            engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "magellan_bonecracker_sub", loc, proj.getFacing(), randomVel);
        }
        int shotCount2 = 1;
        ...
        Vector2f explosion_offset = MagellanUtils.translate_polar(weapon_location, 26.0f, weapon.getCurrAngle());
        Vector2f explosion_offset2 = MagellanUtils.translate_polar(weapon_location, 22.0f, weapon.getCurrAngle());
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if (weapon.isFiring()) {
            ...
            ShipAPI ship = weapon.getShip();
            this.elapsed += amount;
            Vector2f particle_offset = MagellanUtils.translate_polar(weapon_location, 19.0f, weapon.getCurrAngle());
            ...
            for (int x = 0; x < particle_count_this_frame; ++x) {
                ...
                float speed = MagellanUtils.get_random(20.0f, 80.0f);
                float angle = weapon.getCurrAngle() + MagellanUtils.get_random(-18.0f, 18.0f);
