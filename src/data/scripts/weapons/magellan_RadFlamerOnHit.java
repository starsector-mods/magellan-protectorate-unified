    ...
    protected void dealDamage() {
        ...
        for (int j = -2; j <= 2; ++j) {
            for (int k = -2; k <= 2; ++k) {
                ...
                // Extra damage is mitigated by armor
                float effectiveArmor = Math.max(armorInCell, maxArmor * 0.05f);
                float armorReduction = damage / (damage + effectiveArmor);
                armorReduction = Math.max(0.15f, Math.min(1.0f, armorReduction));
                float mitigatedDamage = damage * armorReduction;

                if (mitigatedDamage <= 0.0f) continue;
                mitigatedDamage = Math.min(mitigatedDamage, armorInCell);
                this.target.getArmorGrid().setArmorValue(cx, cy, Math.max(0.0f, armorInCell - mitigatedDamage));
                damageDealt += mitigatedDamage;
            }
        }
        if (damageDealt > 0.0f) {
