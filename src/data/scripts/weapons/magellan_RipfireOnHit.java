...
public class magellan_RipfireOnHit
implements OnHitEffectPlugin {
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        ...
        if (!shieldHit && !projectile.isFading() && target instanceof ShipAPI) {
            ...
            float explosion_damage = projectile.getDamageAmount();
            float nebula_size = 10.0f * (0.75f + (float)Math.random() * 0.5f);

            dealArmorDamage(projectile, (ShipAPI)target, point, explosion_damage);

            engine.addNebulaSmokeParticle(point, v_comp, nebula_size, NEBULA_SIZE_MULT, NEBULA_RAMPUP, 0.3f, FX_DUR, SMOKE_COLOR);
            engine.addNebulaSmokeParticle(point, v_comp, nebula_size, NEBULA_SIZE_MULT, NEBULA_RAMPUP, 0.6f, FX_DUR, SMOKE_COLOR);
