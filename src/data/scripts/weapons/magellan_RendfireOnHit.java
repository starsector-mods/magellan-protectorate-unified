...
public class magellan_RendfireOnHit
implements OnHitEffectPlugin {
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        ...
        if (!shieldHit && !projectile.isFading() && target instanceof ShipAPI) {
            ...
            if (selector <= 0.333f) {
                explosion_damage = projectile.getDamageAmount() / 3.0f;
                explosion_size = 10.0f * (0.75f + (float)Math.random() * 0.5f);
            } else {
                explosion_damage = projectile.getDamageAmount() / 1.5f;
                explosion_size = 15.0f * (0.75f + (float)Math.random() * 0.5f);
            }

            dealArmorDamage(projectile, (ShipAPI)target, point, explosion_damage);

            engine.addNebulaSmokeParticle(point, v_comp, explosion_size, NEBULA_SIZE_MULT, NEBULA_RAMPUP, 0.3f, FX_DUR, SMOKE_COLOR);
            engine.addNebulaSmokeParticle(point, v_comp, explosion_size, NEBULA_SIZE_MULT, NEBULA_RAMPUP, 0.6f, FX_DUR, SMOKE_COLOR);
