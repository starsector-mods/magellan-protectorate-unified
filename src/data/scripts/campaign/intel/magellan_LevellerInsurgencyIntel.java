...
public class magellan_LevellerInsurgencyIntel extends BaseEventIntel {
    ...
    protected String getStageLabel(Object stageId) {
        if (stageId == Stage.REVOLUTION) return "Revolution (Critical)";
        if (stageId == Stage.INSURGENCY) return "Insurgency (Active)";
        return "Agitation (Covert)";
    }

    @Override
    protected String getStageIconImpl(Object stageId) {
        if (stageId == Stage.REVOLUTION) return "graphics/icons/intel/war.png";
        if (stageId == Stage.INSURGENCY) return "graphics/icons/intel/important.png";
    ...
    public void createLargeDescription(CustomPanelAPI panel, float width, float height) {
        float opad = 10f;
        TooltipMakerAPI main = panel.createUIElement(width - 20f, height, true);
        ...
        int bars = 40;
        int filled = (int) Math.round(((double) p / m) * bars);
        ...
        sb.append("] ");
        int pct = (int) (((double) p / m) * 100);
        sb.append(pct).append("%");

        main.addPara("Progress: %s", opad, getBarColor(), sb.toString());
        main.addPara("Points: %s / %s", 3f, Misc.getHighlightColor(), "" + p, "" + m);
        main.addSpacer(opad * 2f);

        for (BaseEventIntel.EventStageData curr : this.getStages()) {
            if (curr.wasEverReached && curr.isOneOffEvent && !curr.isRepeatable) continue;
            this.addStageDescriptionWithImage(main, curr.id);
        }
        this.afterStageDescriptions(main);
        main.addSpacer(opad * 2f);

        if (this.withMonthlyFactors() || this.withOneTimeFactors()) {
            main.addSectionHeading("Event Factors", getFactionForUIColors().getBaseUIColor(), getFactionForUIColors().getDarkUIColor(), Alignment.MID, opad);
            main.addSpacer(opad);
            
            main.addPara("Active Modifiers:", Misc.getHighlightColor(), opad);
            TooltipMakerAPI bullet = main.beginImageWithText("graphics/icons/intel/fleet_log.png", 24f);
            bullet.setBulletedListMode(" • ");
            for (com.fs.starfarer.api.impl.campaign.intel.events.EventFactor factor : this.getFactors()) {
                if (!factor.shouldShow(this)) continue;
                String desc = factor.getDesc(this);
                if (desc != null) {
                    bullet.addPara(desc + ": %s", 3f, factor.getProgressColor(this), factor.getProgressStr(this));
                }
            }
            bullet.setBulletedListMode(null);
            main.addImageWithText(0f);
        }
        
        panel.addUIElement(main).inTL(10f, 0f);
    }
}
