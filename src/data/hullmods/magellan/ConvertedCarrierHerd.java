...
public class ConvertedCarrierHerd extends BaseHullMod {
    ...
    public static final float CR_INCREASE = 50.0f;
    public static final float EXTRA_BAYS = 2.0f;
    public static final float SPEED_REDUCTION = 0.25f;
    ...
    public String getDescriptionParam(int index, HullSize hullSize, ShipAPI ship) {
        float effect = 1.0f;
        if (ship != null && ship.getMutableStats() != null && ship.getMutableStats().getDynamic() != null) {
            effect = ship.getMutableStats().getDynamic().getValue(Stats.DMOD_EFFECT_MULT, 1.0f);
        }
        if (index == 0) return "" + (int) EXTRA_BAYS;
        if (index == 1) return "" + (int) Math.round(SPEED_REDUCTION * 100.0f * effect) + "%";
        if (index == 2) return "" + (int) Math.round(DAMAGE_INCREASE * 100.0f * effect) + "%";
    ...
    public String getDescriptionParam(int index, HullSize hullSize) {
        if (index == 0) return "" + (int) EXTRA_BAYS;
        if (index == 1) return "" + (int) Math.round(SPEED_REDUCTION * 100.0f) + "%";
        if (index == 2) return "" + (int) Math.round(DAMAGE_INCREASE * 100.0f) + "%";
        if (index == 3) return "" + (int) CR_INCREASE + "%";
        if (index == 4) return "" + (int) CARGO_PENALTY;
        return null;
    }
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        tooltip.addSectionHeading("Herd Improvised Flight Deck", herd, herdbg, Alignment.MID, pad);
        tooltip.addPara("- Converts cargo hold into %s built-in fighter bays.", pad, h, "" + (int) EXTRA_BAYS);
        tooltip.addPara("- Improvised manufactory reduces spawned fighter speed by %s and increases damage taken by %s.", padS, bad,
                Math.round(SPEED_REDUCTION * 100.0f * effect) + "%",
                Math.round(DAMAGE_INCREASE * 100.0f * effect) + "%");
