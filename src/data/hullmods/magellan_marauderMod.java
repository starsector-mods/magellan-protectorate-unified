...
public class magellan_marauderMod extends BaseHullMod {
    ...
    private static final Map<HullSize, Float> NAV = new EnumMap<>(HullSize.class);
    static {
    ...
    public void applyEffectsBeforeShipCreation(HullSize hullSize, MutableShipStatsAPI stats, String id) {
        ...
        Float navVal = hullSize != null ? NAV.get(hullSize) : 0f;
        if (navVal != null && navVal > 0f) {
            stats.getDynamic().getMod(Stats.COORDINATED_MANEUVERS_FLAT).modifyFlat(id, navVal);
        }
    ...
    public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, float width, boolean isForModSpec) {
        ...
        tooltip.addPara("- " + getMagellanString("MarauderModDesc2"), padS, h,
                Math.round(NAV.get(HullSize.FRIGATE)) + "%",
                Math.round(NAV.get(HullSize.DESTROYER)) + "%",
                Math.round(NAV.get(HullSize.CRUISER)) + "%",
                Math.round(NAV.get(HullSize.CAPITAL_SHIP)) + "%");
