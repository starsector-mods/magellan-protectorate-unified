...
- **Java 17 LTS Bytecode:** Fully recompiled all mod scripts targeting Java 17 LTS bytecode (`major version: 61`) for native Starsector 0.98a compatibility and optimal performance.
- **Comprehensive Automated Test Suite:** Expanded JUnit 5 and JaCoCo test suites to **155 automated unit and integration tests** covering economics, fleet spawning, campaign intel, console commands, combat listeners, and hullmods.
- **Strict JSON & CSV Compliance:** Sanitized all `.faction`, `.skin`, `.json`, and CSV files (`ship_data.csv`, `weapon_data.csv`, `hull_mods.csv`, `descriptions.csv`), resolving column shifts, malformed arrays, and unquoted comma strings.
- **WorldGen & Scavenge Safety:** Ensured all procedurally generated terrain entities (`DEBRIS_FIELD`, `ASTEROID_BELT`, ring bands) properly register names, preventing null-pointer crashes during campaign Scavenge ability usage.

---

### 📖 Lore & Codex Overhaul
- **271 Rewritten Descriptions:** Fully rewrote and elevated all in-game and codex entries across the entire catalog to professional Starsector vanilla lore standards:
  - **Ships & Hulls (122 entries):** All Protectorate line vessels, Leveller insurgent conversions, Herd scrap warships, Raider/Marauder flagships, and Yellowtail escort hulls.
  - **Strike Craft & Fighters (21 entries):** Detailed tactical doctrines for fighters, bombers, and defense automata.
  - **Weapons (99 entries):** Complete lore, field quotes, and technical specifications for ballistics, masers, cut lasers, electromagnetic solenoids, and missile systems.
  - **Custom Entities & Factions (29 entries):** Lore and descriptions for colonies, outposts, derelicts, stations, and faction alignments.

---

### 💥 Weapons & Combat Mechanics Rework
- **Glowtide SRM & Glowfire MRM Rework:** Reworked corrosive radiation on-hit effects (`magellan_RadFlamerOnHit.java`) so that lingering energy damage is properly mitigated by target armor cells instead of flat unmitigated hull subtraction.
- **Stavros Heavy Bomber Overhaul:** Reworked the Marauder Stavros Bomber with single-shot High Intensity Cutting Lasers with 900 base range for surgical armor penetration.
- **Rusalka Mod-Destroyer Integration:**
  - Integrated 4x built-in **Stunfire SRM Tubes** (`magellan_stunfire_tube`) featuring regenerating ammo (1 missile every 10s) and flush spriteless mounts.
  - Integrated 6x built-in **Multi-Beam PD Nodes** (`magellan_multipdnode`) featuring native Magellan/Leveller electric beam FX and audio.
  - Eliminated hard external mod requirements, allowing the Rusalka to deploy with full native loadouts in standalone play.

---

### 🛡️ Hullmods & S-Mod Modernization
- **Antique & Boss Hullmods:**
  - *Mothership Core:* Modernized player level stepping to align with Starsector 0.98a's maximum level cap (15) with null-safe progression.
  - *Duncan Testbed & Overdrive:* Hardened armor regeneration listeners and subsystem controllers against detached modules and null entities during intense combat.
  - *Logistics Network:* Refactored tooltip rendering and resolved string formatting crashes in fleet preview screens.
- **Converted Carriers & Flight Decks:**
  - Overhauled *Converted Shuttle Bay*, *Converted Carrier (Leveller)*, *Converted Carrier (The Herd)*, *Maizan Bay*, and *Spacious Hangars* with standardized S-mod integration and size-dependent lookup tables.
- **0.98a Story Point S-Mod Integration:**
...
### 🗺️ Campaign & Intel Systems
- **Leveller Insurgency & Necksnapper Progression:** Reworked intel event displays with clean non-overlapping typography, text-based progress trackers, and precise stage ETA countdowns.
...
- **Story Point Dialogue Actions:** Integrated vanilla-style Story Point interactions at Crucible Base and Rosebriar Station for major reputation gains.
...
- **High-Fidelity Audio Compression:** Re-encoded music soundtrack tracks in `sounds/music/` using Vorbis Quality 5 (~160 kbps VBR), reducing music folder size from **35.7 MB down to 13.1 MB (-63.3% / 22.6 MB saved)** with transparent, CD-grade audio quality.
- **Dead Asset Cleanup:** Removed unreferenced legacy test files, temporary build folders, and orphaned graphics.
- **Lightweight Distribution:** Build pipeline outputs cleanly to `jars/Magellan.jar` with automatic intermediate `target/` purging.
